...
def mock_llm():
    ...
    llm = MagicMock()
    response = MagicMock()
    response.content = "LLM refined text"
...
class TestLLMEnhancement:
    ...
    def test_llm_success_path(self, mock_settings_with_llm, mock_llm, sample_chunk):
        ...
        response = MagicMock()
        response.content = "Beautifully refined text by LLM"
...
class TestConfiguration:
    ...
    def test_lazy_llm_initialization(self, mock_settings_with_llm):
        ...
        with patch('src.ingestion.transform.chunk_refiner.LCLLMFactory.create') as mock_factory:
    ...
    def test_llm_init_failure_disables_llm(self, mock_settings_with_llm):
        ...
        with patch('src.ingestion.transform.chunk_refiner.LCLLMFactory.create', side_effect=Exception("Init failed")):
            refiner = ChunkRefiner(mock_settings_with_llm)
