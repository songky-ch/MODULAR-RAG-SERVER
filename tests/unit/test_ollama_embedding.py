...
class TestOllamaEmbeddingFactoryIntegration:
    """Test suite for Ollama Embedding factory integration (LangChain-backed)."""

    def test_factory_creates_langchain_embedding(self, mock_settings_ollama: Any) -> None:
        """Test that factory creates a LangChainEmbeddingAdapter for ollama."""
    ...
    def test_factory_returns_base_embedding_interface(self, mock_settings_ollama: Any) -> None:
        """Test that factory result satisfies BaseEmbedding interface."""
        from unittest.mock import patch, MagicMock
        from src.libs.embedding.base_embedding import BaseEmbedding

        mock_lc_emb = MagicMock()
        mock_lc_emb.embed_documents.return_value = [[0.1, 0.2, 0.3]]
        with patch(
            "src.libs.lc_llm.LCLLMFactory.create_embedding",
            return_value=mock_lc_emb,
        ):
