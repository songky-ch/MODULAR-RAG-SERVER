...
class MetadataEnricher(BaseTransform):
    ...
    def llm(self) -> Optional[BaseChatModel]:
        """Lazy-load LangChain LLM instance."""
        if self.use_llm and self._llm is None:
            try:
                self._llm = LCLLMFactory.create(self.settings)
                logger.info("LangChain LLM initialized for metadata enrichment")
    ...
    def _llm_enrich(
        ...
        try:
            ...
            if not response_text or not response_text.strip():
                logger.warning("LLM returned empty content for metadata enrichment")
                return None
