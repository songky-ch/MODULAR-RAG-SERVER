...
class VectorUpserter:
    ...
    def upsert(
        ...
        for chunk, vector in zip(chunks, vectors):
            ...
            # 管理面板和 data_service 需要通过元数据中的 doc_hash 查询分块。
            # 该值通常来自完整性检查器或由上游传入；缺失时根据 source_path 计算。
            doc_hash = chunk.metadata.get("doc_hash")
            if not doc_hash:
                import hashlib
                source_path = chunk.metadata.get("source_path", "")
                doc_hash = hashlib.sha256(source_path.encode("utf-8")).hexdigest()
            ...
            record = {
                ...
                "metadata": {
                    **chunk.metadata,  # Preserve all original metadata
                    ...
                    "doc_hash": doc_hash,  # Required by DataBrowser to filter chunks
                },
            }
