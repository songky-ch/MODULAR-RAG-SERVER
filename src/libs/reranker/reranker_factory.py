...
def _lazy_import_api_reranker():
    """Lazy import to avoid circular dependencies."""
    from src.libs.reranker.api_reranker import APIReranker
    return APIReranker
...
class RerankerFactory:
    ...
    def create(cls, settings: Settings, **override_kwargs: Any) -> BaseReranker:
        ...
        # Lazy register API reranker if not already registered
        if "api" not in cls._PROVIDERS:
            APIReranker = _lazy_import_api_reranker()
            cls.register_provider("api", APIReranker)
